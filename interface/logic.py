...
#------------------------------------------------------------------------------------
# start block
# Превращаем стек [[50,50,50],......]
# В ряды, пригодные для рисования диаграмм ганта.
#------------------------------------------------------------------------------------
...
def run(sets:tuple, details_set:list)->list:
    """ Функция для запуска блока формирования из сета массива для
    построения диаграмм ганта.
    sets:        парамеры сета ( колличество деталей , колличество станков)
    detail_set:  сам сет типа [[50,50,50],...]
    return :  [[1,0,0,...],[0,1,0,...],[2,0,0,...],....]
    """
    AA = details_set
    ...
    for j in range(len(massiv[0])):
        ...
        for z in range(len(stec)):
            ...
            if stec[z] < a and stec[z] != 0:
                print("Результат отрицательный", z, j, a, stec[z])
                return False

    print("по первому критерию всё ок")
    return True
